  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "O mnie — Paula Janowska-Kiełkiewicz, Tłumacz przysięgły" },
      {
        ...
        content: "Poznaj historię, doświadczenie i pasję tłumacza przysięgłego",
      },
      ...
      >
        <div className="mx-auto max-w-6xl px-6">
          <div className="mt-12 grid gap-10 lg:grid-cols-[0.95fr_1.05fr] items-center">
            <div
              className={`relative overflow-hidden rounded-[2rem] border border-border bg-card shadow-elegant transition-all duration-700 ${isVisible ? "animate-slide-up" : ""}`}
            >
              <img
                src={deskImage}
                alt="Biurko"
            ...
            <div className="space-y-8">
              ...
              <h2
                className={`font-display text-4xl text-gold transition-all duration-700 ${isVisible ? "animate-slide-up" : ""}`}
              >
                {t.about.title}
              </h2>
              <div
                className={`space-y-6 text-lg leading-8 text-muted-foreground transition-all duration-700 ${isVisible ? "animate-slide-up" : ""}`}
              >
                <p>{t.about.p1}</p>
                <p>{t.about.p2}</p>
              </div>
              ...
              <div
                className={`grid gap-4 sm:grid-cols-3 transition-all duration-700 ${isVisible ? "animate-slide-up" : ""}`}
              >
                {t.about.list.map((fact, index) => {
                  const url =
                    index === 0
                      ? "https://arch-bip.ms.gov.pl/pl/rejestry-i-ewidencje/tlumacze-przysiegli/lista-tlumaczy-przysieglych/translator,661.html"
                      : undefined;
                  const cardClasses =
                    "group block rounded-3xl border border-border bg-card p-6 transition-all duration-500 hover:bg-accent/40 hover:border-foreground/20 hover:shadow-sm";
                  const innerContent = (
                    <>
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground flex items-center gap-1.5">
                        {fact.label}
                        {url && (
                          <span className="inline-block transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 text-primary text-sm font-light">
                            ↗
                          </span>
                        )}
                      </p>
